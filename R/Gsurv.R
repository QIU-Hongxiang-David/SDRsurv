...
#' @param nfold number of folds in sample splitting when estimating the survival curves of time to event in each time window. Default is 1 corresponding to no sample splitting.
...
Gsurv<-function(
    ...
    ############################################################################
    # G-computation transformation (create a nested list: tvals > check.in.times)
    ############################################################################
    stagewise.pseudo.outcomes<-Gtransform(follow.up.time,pred_event.list,check.in.times,tvals,truncation.index,id.var,time.var,event.var)
