...
#' @return a list containing three lists of fitted `SuperLearner` models corresponding to each t in `tvals`. The three elements correspond to SDR, G-computation and IPW, respectively. If `Q.formula` is empty, then return a list of numbers, each being estimated P(T <= t) for t in `tvals`.
...
SDR_G_IPW_surv<-function(
    ...
    ############################################################################
    # transformations (create a nested list: tvals > check.in.times)
    ############################################################################
    SDR.stagewise.pseudo.outcomes<-SDRtransform(follow.up.time,pred_event_censor.list,check.in.times,tvals,truncation.index,id.var,time.var,event.var)
    G.stagewise.pseudo.outcomes<-Gtransform(follow.up.time,lapply(pred_event_censor.list,function(x) x$event),check.in.times,tvals,truncation.index,id.var,time.var,event.var)
    IPW.stagewise.pseudo.outcomes<-IPWtransform(follow.up.time,lapply(pred_event_censor.list,function(x) x$censor),check.in.times,tvals,truncation.index,id.var,time.var,event.var)
