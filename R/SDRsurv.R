...
#' @param censor.formula a list of formulas to specify covariates being used when estimating the conditional survival probabilities of time to censoring at each check-in time. Similar to `event.formula`. If `event.method` is chosen as `"survSuperLearner"`, `censor.formula` is not used and `event.formula` is used instead.
...
SDRsurv<-function(
    ...
    ############################################################################
    # sequentially doubly robust transformation and regression
    ############################################################################
    SDRreg.SuperLearner(covariates,follow.up.time,pred_event_censor.list,check.in.times,tvals,truncation.index,id.var,time.var,event.var,Q.formula,Q.SuperLearner.control,denom.survival.trunc)
}
